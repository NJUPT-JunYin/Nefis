...
public class NCUtils {
    ...
    public static byte[] test(byte[] a, byte[] b){
        byte[] c = new byte[3];
        Multiply3(a,1,3,b,3,3,c);
        //对于异或运算的成功 使得即时解码称为肯能
        return c;
    }
    ...
    //对于即时解码的编码算法
    private static byte[] reencode2(byte[] encodeData, int row, int col, byte[] result) {
        //1*row 随机矩阵  与  row * col的数据矩阵相乘
        byte[] randomMatrix = new byte[row];
        //这里只需要生成一个空矩阵即可
        Multiply2(randomMatrix, 1, row, encodeData, row, col, result);
        return result;
        //这个result 只有1400字节 只是一个全部行异或之后的结果
    }
    /**
     * 解码
     *
...
//    }
    ...
    public static int getRank1(byte[] matrix ,int row,int col) {
    ...
    //矩阵相乘
    public static native void Multiply3(byte[] matrix1, int row1, int col1, byte[] matrix2, int row2, int col2, byte[] ret);
}
