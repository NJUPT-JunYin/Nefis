...
//矩阵异或运算
JNIEXPORT void JNICALL
Java_nc_NCUtils_Multiply3(JNIEnv *env, jobject instance,
                          jbyteArray matrix1, jint row1, jint col1,
                          jbyteArray matrix2, jint row2, jint col2,
                          jbyteArray ret);
...
Java_nc_NCUtils_Multiply3(JNIEnv *env, jobject instance,
    ...
    unsigned char temp = 0;
    for (int i = 0; i < row1; ++i) {
        for (int j = 0; j < col1; ++j) {//row 行 col列
            temp = 0;
            for (int k = 0; k < row2; ++k) {
                //temp = gf_add(temp, gf_add(pData1[i * col1 + j], pData2[k * col2 + j]));
               temp = gf_add(temp , pData2[k * col2 + j]);
            }
            pResult[i * col2 + j] = (jbyte) temp;
        }
    }
